...
public class BuildPickerScreen extends Screen {
    ...
    private Set<String>  sessionHasChat = new HashSet<>();   // scripts that have a saved chat
    ...
    public BuildPickerScreen(Screen parent, Consumer<String> onOpen, Consumer<String> onNew) {
        super(Component.literal("Open Script"));
        this.parent = parent;
        this.onOpen = onOpen;
        this.onNew  = onNew;
    }
    ...
    protected void init() {
        ...
        nameField = new EditBox(font,
            px + PAD, by, W - PAD * 2 - 82, 14, Component.empty());
        nameField.setMaxLength(64);
        nameField.setHint(Component.literal("new_script.py"));
        nameField.setCanLoseFocus(true);
        addRenderableWidget(nameField);

        // New button
        addRenderableWidget(Button.builder(Component.literal("New"), btn -> pickNew())
            .bounds(px + W - PAD - 76, by - 1, 36, 16).build());

        // Cancel button
        addRenderableWidget(Button.builder(Component.literal("Cancel"),
                btn -> minecraft.setScreen(parent))
            .bounds(px + W - PAD - 37, by - 1, 37, 16).build());
    }

    private void loadSessions() {
        sessions.clear();
        sessionHasChat.clear();

        // Find which scripts have a saved chat file
        try {
            if (Files.exists(BuildSession.CHATS_DIR)) {
                Files.list(BuildSession.CHATS_DIR)
                    .filter(p -> p.toString().endsWith(".json"))
                    .forEach(p -> {
                        try {
                            var obj = JsonParser.parseString(Files.readString(p)).getAsJsonObject();
                            if (obj.has("script")) sessionHasChat.add(obj.get("script").getAsString());
                        } catch (Exception ignored) {}
                    });
            }
        } catch (Exception ignored) {}

        // List every .py in the scripts directory (excluding mc.py library)
        try {
            if (Files.exists(ScriptRunner.SCRIPTS_DIR)) {
                Files.walk(ScriptRunner.SCRIPTS_DIR)
                    .filter(p -> p.toString().endsWith(".py"))
                    .filter(p -> !p.getFileName().toString().equals("mc.py"))
                    .map(p -> ScriptRunner.SCRIPTS_DIR.relativize(p).toString().replace('\\', '/'))
                    .sorted()
    ...
    private void confirmDelete() {
        ...
        sessionHasChat.remove(deleteConfirmName);
    ...
    /** Returns true if (mx, my) is inside the Del button for the given row (only rows with chat). */
    private boolean inDelBtn(int row, double mx, double my) {
        if (row < 0 || row >= sessions.size()) return false;
        if (!sessionHasChat.contains(sessions.get(row))) return false;
        int ry  = listY0 + row * ROW - (int) scrollPx;
        int bx  = delBtnX();
        int by  = ry + (ROW - DEL_H) / 2;
        return mx >= bx && mx < bx + DEL_W && my >= by && my < by + DEL_H
            && ry >= listY0 && ry + ROW <= listY1 + ROW;
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        ctx.centeredText(font,
            Component.literal("Open Script"), x0 + W / 2, y0 + 3, C_ORANGE);
        ...
        if (sessions.isEmpty()) {
            ctx.text(font,
                Component.literal("No scripts found in scripts folder."),
            ...
            for (int i = 0; i < sessions.size(); i++) {
                ...
                boolean hasChat = sessionHasChat.contains(sessions.get(i));
                boolean hovered = (i == hoveredRow && i != hoveredDelRow);

                // Row hover highlight
                if (hovered) ctx.fill(innerX0, ry, innerX1, ry + ROW - 1, C_HOVER);

                // Script name — truncate to leave room for chat dot / Del button
                int nameMaxX = hasChat ? dbx - 4 : innerX1 - 4;
                String name = sessions.get(i);
                if (font.width(name) > nameMaxX - innerX0 - 3)
                    name = font.plainSubstrByWidth(name, nameMaxX - innerX0 - 6) + "…";
                ctx.text(font, Component.literal(name),
                    innerX0 + 3, ry + 4,
                    hovered ? C_WHITE : (hasChat ? C_GREY : C_DIM));

                // Del button — only for scripts with a saved chat
                if (hasChat) {
                    ...
                    ctx.text(font, Component.literal("Del"),
                        dbx + (DEL_W - font.width("Del")) / 2, dby + 2,
                        C_WHITE, false);
                }
            }
        }
    ...
    private void renderDeleteConfirm(GuiGraphicsExtractor ctx, int mx, int my) {
        ...
        ctx.text(font, Component.literal("§cDelete chat history?"),
        ...
        ctx.text(font, Component.literal("§7This cannot be undone."), dlgX + 8, dlgY + 28, C_DIM, false);
